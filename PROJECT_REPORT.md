...
The high-level architecture of MedGuardian AI is designed around an **Agent-Inspired Service Orchestration Pattern**. Instead of monolithic procedural scripts, the platform orchestrates task execution using a central **Coordinator Service** coordinating specialized **Modular Service Agents** (encapsulated as Django services). These modules interact with databases and external clinical endpoints via clean abstraction layers.

The architectural diagram below illustrates this layered routing:

![System Architecture](./docs/system_architecture.png)

### 3.1.1 Service Core Component Specifications
- **Coordinator Service (risk_engine.py)**: The orchestrator of the safety evaluation. It receives signals from patient profile updates or medication changes, manages the background evaluation thread, dynamically coordinates the service agents, aggregates outputs, and executes the Gemini synthesis loop.
- **Prescription Ingestion Agent (ingestion.py)**: Specializes in parsing prescription images. Invokes Gemini 2.5 Flash multimodal vision capabilities to run OCR, execute Named Entity Recognition (NER), and map text blocks to structured Pydantic models.
- **Medication Cabinet Agent (patients/models.py)**: Manages medication cabinet entries and updates. Tracks active vs. inactive cabinet states and patient demographics.
- **RAG Evidence Agent (rag.py)**: Executes Retrieval-Augmented Generation (RAG) semantic vector queries against a local ChromaDB store. Falls back to localized keyword queries during offline development.
- **Clinical Risk Agent (risk_engine.py)**: Coordinates medical safety auditing. Converts generic drug names to RxCUI concepts using the NLM RxNav API, retrieves drug-drug interactions, and queries openFDA label warnings. Features a deterministic local safety fallback rule-gate.
- **Report Agent (reports.py)**: Uses ReportLab programmatically to generate clinician safety dossiers and patient summaries as printable PDFs.
- **Chat Agent (views.py / consumers.py)**: Drives the clinical chat panel, enabling patients to ask natural language questions grounded in local guidelines.
- **Clinical API Gateway Service**: Providing a unified interface to communicate with local resources (SQLite, ChromaDB), clinical APIs (NLM RxNav, openFDA), and generative foundation models.
...
4. **Coordinator Initialization**: The Coordinator Service is instantiated, reading the active patient cabinet and demographic variables.
5. **Guideline & Label Retrieval**: The Coordinator invokes the *RAG Evidence Agent* and *Clinical Risk Agent* in parallel. The *RAG Evidence Agent* generates vector query embeddings to fetch relevant safety guidelines from ChromaDB, while the *Clinical Risk Agent* queries NLM RxNav and openFDA tools to gather chemical interaction matrices and official warning labels.
...
| Task Orchestrator | Custom Django Signals & Celery |
| Vector Database | ChromaDB                       |
| Embeddings      | Gemini text-embedding-004      |
| API Abstraction | Django REST Service Layer      |
